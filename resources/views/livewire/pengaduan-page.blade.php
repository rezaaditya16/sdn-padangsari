<div>
    ...
    @if($student)
        ...
        @if($student)
            <div class="max-w-3xl mx-auto px-6 pb-20">
                <div class="bg-white shadow-lg border rounded-3xl p-8 text-center">
                    <div class="mb-6">
                        <svg class="w-16 h-16 text-blue-600 mx-auto mb-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path>
                        </svg>
                        <h3 class="text-xl font-bold text-gray-800 mb-2">Riwayat Pengaduan</h3>
                        <p class="text-gray-600">
                            @if($pengaduanHistory && $pengaduanHistory->count() > 0)
                                Anda memiliki {{ $pengaduanHistory->count() }} pengaduan terdaftar
                            @else
                    ...
                    <button wire:click="viewAllPengaduan" 
                            class="inline-flex items-center px-6 py-3 bg-blue-600 text-white font-semibold rounded-xl shadow-lg hover:bg-blue-700 transition-all duration-300 transform hover:scale-105">
...
<script>
    ...
    document.addEventListener('livewire:init', () => {
        Livewire.on('pengaduan-submitted', () => {
            // Scroll ke history button setelah submit
            setTimeout(() => {
                const historyButton = document.querySelector('[wire\\:click="viewAllPengaduan"]');
                if (historyButton) {
